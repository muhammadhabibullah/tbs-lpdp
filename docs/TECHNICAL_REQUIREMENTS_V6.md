...
| AP-9 | Offline feature deltas, applied only under `VITE_OFFLINE`: no `MaintenanceGate` or `HumanVerificationGate` (neither concept applies), package statistics are computed locally and labeled *"Statistik lokal (perangkat ini)"*, and the report-question dialog (v2) stores the report locally **and** offers *"Kirim via Email"* using the existing prefilled `feedbackMailto()` so reports can still reach the maintainer. The web menu item added by FE-42 is hidden in the app. |
