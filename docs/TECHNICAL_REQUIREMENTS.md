...
| FE-19 | **Capacity state** (added for the public launch): when `get_service_status()` reports `accepting_attempts: false` (BE-18), every **Mulai Try Out** button is disabled and relabelled **Kuota Penuh**, with one `notice warn` banner above the package list explaining why and that running attempts and old Pembahasan are unaffected. A failed status probe means "open" — the server stays the authority — and the `P0007` raised by a click that races the ceiling shows the *same* copy as the banner. Design: [CAPACITY_GUARD.md](CAPACITY_GUARD.md). |
...
| NF-10 | **Retention** (added for the public launch): `supabase/maintenance.sql` schedules two daily `pg_cron` sweeps — attempts started more than 7 days ago (cascading to their sections, answers and events), and anonymous `auth.users` untouched for 60 days that filed no question report. The 7-day window is stated to users in the site footer, so the two must stay in sync. Operational only: nothing in the app reads the jobs, and re-applying the schema files never undoes them. Where BE-16/BE-17 bound how fast the database grows, these bound its total. |
| NF-11 | **Capacity headroom** (added for the public launch): the storage ceiling that BE-18 enforces is a **soft** one — 400 MB of the free tier's 500 by default — chosen so that attempts already running have room to finish and NF-10's daily sweep has room to reclaim. It lives in a table row, not in code, so raising it is one `update` in the SQL editor with no redeploy. Measurement is O(1) (`pg_database_size` + `pg_class.reltuples`), cached for 5 minutes, and re-measured lazily on read so the guard is correct even where `pg_cron` was never enabled. Design: [CAPACITY_GUARD.md](CAPACITY_GUARD.md). |
