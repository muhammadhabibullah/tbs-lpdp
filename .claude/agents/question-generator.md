...
1. Read `.agents/skills/lpdp-question-generation/SKILL.md` — it defines the JSON format, blueprint, and directory layout. It is the contract; follow it exactly.
2. Read `questions/schema.json` and one existing question file (if any) as a concrete example.
3. Check `questions/bank/<package>/` for what already exists so you continue numbering instead of colliding.
...
- For computable types you MUST generate via the Python scripts in `questions/generator/` — never hand-compute a key for these:
  ...
  | Type | Script |
  |------|--------|
  | `deret_angka` | `deret_angka.py` (add `--blanks 2` for next two terms, `--leading` for missing first term, `--interior` for middle blanks, or `--template ...`) |
  | `deret_huruf` | `deret_huruf.py` (add `--blanks 2` or `--interior` for square positions) |
  | `aritmetika`, `perbandingan_kuantitatif` | `aritmetika.py --type ...` |
  | `aljabar` | `aljabar.py` |
  | `kecukupan_data` | `kecukupan_data.py` (add `--kind geometry` for figure families) or `kecukupan_data_predikat.py` (yes/no inequality sufficiency) |
  ...
  `kecukupan_data` and `peluang_kombinatorik` exist as scripts precisely because they are the two types humans key wrongly most often: sufficiency is a claim about three facts at once, and probability keys drift between ordered and unordered counting. If you ever find yourself reasoning out one of these keys in prose, stop and run the script.
...
Always run `python3 questions/generator/validate_bank.py` and fix every reported error until it exits 0. Report back: how many questions written per subtest/type, validation status, and anything you flagged as needing human review.

You write files and run scripts, but you never push to Supabase — the developer does that with `push_to_supabase.py` after review.
